...
class StoryChiefMappingHandler {
  ...
  public function map(): Entry {
    $field_mapping = array_filter(Arr::dot(config('storychief.mapping', [])));

    foreach ($field_mapping as $storychief_field => $statamic_field_handle) {
      $payload_value = $this->getPayloadFieldValueByKey($storychief_field);
      $statamic_field = $this->getStatamicFieldByHandle($statamic_field_handle);
  ...
  protected function setFieldValue(Field $field, $value): void {
    ...
    $field_handler_class = config("storychief.fieldtypes.$field_type", NULL);

    if(is_null($field_handler_class)) {
      throw new InvalidFieldTypeException("No field handler for type '$field_type'");
    }

    $class = new ReflectionClass($field_handler_class);
    if (!$class->implementsInterface(FieldHandlerInterface::class) ) {
      throw new InvalidFieldTypeException("Field handler '$field_handler_class' should implement interface ".FieldHandlerInterface::class);
    }

    $field_handler = new $field_handler_class($this->entry, $field, $this->payload, $value);
  ...
  protected function getPayloadFieldValueByKey(string $key) {
    if (explode('.', $key)[0] === 'custom_fields') {
      ...
      return Arr::get(Arr::first($custom_fields,
    ...
    switch ($key) {
      ...
      case 'author_name':
        $first = Arr::get($this->payload, 'data.author.data.first_name', '');
        $last = Arr::get($this->payload, 'data.author.data.last_name', '');
        return trim("$first $last");
      case 'featured_image':
        return Arr::get($this->payload, 'data.featured_image.data.sizes.full',
          NULL);
      case 'tags':
      case 'categories':
        return array_column(Arr::get($this->payload, "data.$key.data", []),
          'name');
      default:
  ...
  protected function getStatamicFieldByHandle(string $handle): Field {
    ...
    if (!$field) {
      throw new InvalidFieldHandleException("Could not find any field with handle '$handle'");
    }
