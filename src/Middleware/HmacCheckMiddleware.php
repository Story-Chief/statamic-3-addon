...
class HmacCheckMiddleware {
  ...
  public function handle(Request $request, Closure $next) {
    ...
    $calc_mac = hash_hmac('sha256', json_encode($payload), config('storychief.encryption_key'));

    if (!hash_equals($given_mac, $calc_mac)) {
      throw new UnauthorizedHttpException(401, 'Invalid payload hash, please check your encryption key.');
    }
